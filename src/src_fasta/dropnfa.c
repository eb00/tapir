...
/* $Name: fa34t25d9 $ - $Id: dropnfa.c,v 1.77 2006/06/01 16:05:30 wrp Exp $ */
...
/*
  ...
  12-Nov-2004 - replace -DGAP_OPEN with -DOLD_FASTA_GAP - reverse meaning
...
#ifndef ALLOCN0
static char *verstr="3.49 May 2006";
#else
static char *verstr="3.49an0 May 2006";
#endif
...
/* Conquer: recursively around midpoint */
  ...
  if (type == 1) {
    nw_align(A,B,midi,midj,tb,-g,w,g,h,f_str);
    nw_align(A+midi,B+midj,M-midi,N-midj,-g,te,w,g,h,f_str);
  }
  else {
    nw_align(A,B,midi-1,midj,tb,0,w,g,h,f_str);
    DEL(2);
    nw_align(A+midi+1,B+midj,M-midi-1,N-midj,0,te,w,g,h,f_str);
  }
  return midc;
}
...
{ 
  ...
  c = nw_align(A,B,M,N,-G,-G,W,G,H,f_str);	/* OK, do it */
...
static int BCHECK_SCORE(const unsigned char *A, const unsigned char *B,
		       int M, int N, int *S, int **w, int g, int h,
		       int *nres)
{ 
  register int   i,  j, op, nc;
	 ...
	 struct bdstr *bss)
{
  ...
  if (r == -1) { /* optimal alignment did not cross the middle diagonal */
    if (rmid < 0) bg_align(A,B,M,N,rmid+1,up,tb,te,w,g,h,bss);
    else bg_align(A,B,M,N,low,rmid-1,tb,te,w,g,h,bss);
  } else {
    k = r;
    l = FP[k];
    kt = FT[k];
    ...
    /* first block */
    if (rmid < 0) {
      bg_align(A,B,r-1,r+rmid,rmid+1,min(up,r+rmid),tb,1,w,g,h,bss);
      DEL(1)
    } else if (rmid > 0) {
      bg_align(A,B,r,r+rmid-1,max(-r,low),rmid-1,tb,2,w,g,h,bss);
      INS(1)
    }
	...
	INS(1)
	t1 = l-k-1;
	bg_align(A+k,B+k+rmid+1,t1,t1,0,min(t1,t2),2,1,w,g,h,bss);
	DEL(1)
	} else { /* kt == 2, left-hand side triangle */
	  DEL(1)
	  t1 = l-k-1;
	  bg_align(A+k+1,B+k+rmid,t1,t1,max(-t1,t3),0,1,2,w,g,h,bss);
	  INS(1)
	  }
    }
    ...
    if (N-M > rmid) {
      INS(1)
      t1 = k+rmid+1;
      bg_align(A+k,B+t1,M-k,N-t1,0,min(N-t1,t2),2,te,w,g,h,bss);
    } else if (N-M < rmid) {
      DEL(1)
      t1 = M-(k+1);
      bg_align(A+k+1,B+k+rmid,t1,N-(k+rmid),max(-t1,t3),0,1,te,w,g,h,bss);
    }
  }
  return(v);
}
...
{ 
  ...
  c = bg_align(A,B,M,N,low,up,0,0,W,G,H,bss);
