...
/* $Name: fa34t25d9 $ - $Id: nmgetlib.c,v 1.33 2006/05/19 16:30:11 wrp Exp $ */
	...
	int outtty, struct lmf_str *om_fptr)
{
 ...
 l1:
  ...
  if (!opnflg) {	/* here if open failed */
    ...
    rline[0]='\0';
    sprintf(prompt," cannot open %s\r Select library filename",sname);
    STFileDlog(prompt,&freply,llist,LLN);
    if (freply.sfGood==TRUE) {
      PtoCstr((StringPtr)freply.sfFile.name);
      strncpy(sname,(char *)freply.sfFile.name,sizeof(sname)-1);
      sname[sizeof(sname)-1]='\0';
...
#ifdef __MWERKS__
    if (!use_stdin &&
	(libtype==EMBLSWISS || libtype==VMSPIR ||libtype==FULLGB)) {
      fgets(m_fptr->lline,MAX_STR,libf);
      m_fptr->lfflag = (m_fptr->lline[0]==LFCHAR);
      FSEEK(libf,0,0l);
      m_fptr->lpos = 0;
      fgets(m_fptr->lline,MAX_STR,libf);
      if (m_fptr->lfflag) {
	getc(libf);
	/*	fprintf(stderr," m_fptr->lfflag is set\n");  */
      }
    }
#endif
  }
