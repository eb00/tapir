...
/* $Name: fa34t25d9 $ - $Id: mshowbest.c,v 1.42 2006/05/31 15:31:45 wrp Exp $ */
...
l1:
  ...
  if (pst.zsflag>=0) {
    if (m_msg->z_bits==1)
      fprintf (fp, " %5.1f %7.2g",zs_to_bit(bbp->zscore,m_msg->n0,bbp->n1),bbp->escore);
    else
      fprintf (fp, " %6.1f %7.2g",bbp->zscore,bbp->escore);
  }
  show_aux(fp,bbp);
