...
/* $Name: fa34t25d9 $ - $Id: initfa.c,v 1.138 2006/05/24 20:29:45 wrp Exp $ */
...
char *verstr="version 3.4t25 Sept 2, 2005";
...
double
get_lambda(int **pam2p, int n0, int nsq, unsigned char *aa0) {
  double lambda, H;
  double *pr, tot, sum;
...
read_pssm(unsigned char *aa0, int n0, int nsq,
  ...
  for (qi = 0 ; qi < n0 ; qi++) {
	...
	freq2d[qi][rj] = ppst->pam2[0][aa0[qi]][rj+1];
      }
    }
  }
  ...
  scale = 1.0;
  lambda = get_lambda(ppst->pam2[0], 20, 20, ustandard_aa);
  ...
  for (i = 0 ; i < n0 ; i++) {
    fprintf(stderr, "%4d %c:  ", i+1, query[i]);
    for (j = 1 ; j <= 20 ; j++) {
      fprintf(stderr, "%4d", pam2p[i][j]);
    }
    fprintf(stderr, "\n");
  }
  */
