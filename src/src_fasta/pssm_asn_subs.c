...
/* $Name: fa34t25d9 $ - $Id: pssm_asn_subs.c,v 1.8 2006/05/24 20:29:45 wrp Exp $ */
...
static char ncbieaatoa[27] = {"-ABCDEFGHIJKLMNOPQRSTUVWXYZ"};
...
chk_asn_buf(struct asn_bstruct *asnp, int v) {
  ...
  if (asnp->abp + v > asnp->buf_max) {
    ...
    asnp->abp = asnp->buf;
    new = ASN_BUF - asnp->len;
    
    if (!feof(asnp->fd) && 
	(new=fread(asnp->buf + asnp->len, sizeof(char), new, asnp->fd)) != 0) {
      asnp->len += new;
    }
...
get_astr_str(struct asn_bstruct *asnp, char *text, int t_len) {
  ...
  if (*asnp->abp++ != ASN_IS_STR) { /* check for str */
    ...
    asnp->abp = chk_asn_buf(asnp,v_len+2);

    memcpy(text, (char *)asnp->abp, min(v_len, t_len));
    text[min(v_len, t_len)] = '\0';

    asnp->abp += v_len+2; /* skip over last null */
  }
  return asnp->abp;
}
...
get_astr_octstr(struct asn_bstruct *asnp,
  ...
  if (*asnp->abp++ == ASN_NCBIeaa) {
    ...
    asnp->abp = chk_asn_buf(asnp,q_len+2);

    memcpy(oct_str, asnp->abp, min(q_len, o_len));
    oct_str[min(q_len,o_len)]='\0';

    asnp->abp += q_len + 2;	/* skip characters and NULL's */
  }
  return asnp->abp;
}
...
unsigned char *
get_pssm2_freqs(struct asn_bstruct *asnp,
		double ***freqs,
		int n_rows,
		int n_cols) {
  ...
  *freqs = my_freqs;
  ...
  if (asnp->abp[0] == ASN_PSSM2_LAMBDA ) {
    asnp->abp += 2;
    asnp->abp = get_astr_packedfloat(asnp,&f_val);
  }
...
parse_pssm2_asn(struct asn_bstruct *asnp,
  ...
  while (*asnp->abp != '\0' ) {
	...
	asnp->abp = get_pssm2_freqs(asnp, freqs, *n_rows, *n_cols);
	(*n_cols)--;
      }
      else {
	fprintf(stderr, " have frequencies but not rows/cols: %d %d\n",*n_rows, *n_cols);
      }
      break;
...
parse_pssm_asn(FILE *afd,
  ...
  if ((asn_str.buf = calloc(ASN_BUF, sizeof(char))) == NULL ) {
...
parse_pssm_asn_fa( FILE *fd, 
  ...
  ret_val = parse_pssm_asn(fd, &gi, name, acc, descr, query, &nq,
			   n_rows, n_cols, freq2d,
			   &pseudo_cnts, matrix, gap_open, gap_extend);

  if (ret_val <=0) return ret_val;

  /* transform the frequencies */

  for (qi = 0; qi < *n_cols; qi++) {
    for (rj = 0; rj < 26; rj++) { tmp_freqs[rj] = (*freq2d)[qi][rj];}
    for (rj = 0; rj < 20; rj++) { (*freq2d)[qi][rj] = tmp_freqs[pssm_aa_order[rj]];}
  }
  return 1;
}
