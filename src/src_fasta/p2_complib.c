...
/* $Name: fa34t25d9 $ - $Id: p2_complib.c,v 1.85 2006/06/02 02:06:21 wrp Exp $ */
...
#ifdef PVM_SRC
#include "pvm3.h"
char *mp_verstr="v3.4t25, Nov 12, 2004 PVM";
#endif

#ifdef MPI_SRC
#include "mpi.h"
char *mp_verstr="v3.4t25, Nov 12, 2004 MPI";
#endif
...
#ifdef PVM_SRC
  strncpy (workerpgm, WORKERPGM,sizeof(workerpgm));
  strncat(workerpgm, pgm_abbr, sizeof(workerpgm)-strlen(workerpgm));
#endif
...
/*  if (nnodes < 2 ) nnodes = 4; */
  ...
  strcpy(nodefile,pgmdir);
  strcat(nodefile,workerpgm);
  ...
  if ((aa00 = (unsigned char *) malloc ((MAXTST + SEQ_PAD)* sizeof (char))) == NULL)
    s_abort ("Unable to allocate query sequence", "");
  
  if ((aa01 = (unsigned char *) malloc ((MAXTST + SEQ_PAD) * sizeof (char))) == NULL)
    s_abort ("Unable to allocate query sequence", "");
  ...
  */
  ...
  if (m_msg0.maxn <= 0) m_msg0.maxn = MAXLIB;
	...
	for (i=0; i<n1; i++)
	  if (aa1[i]>pst.nsq) {
	    fprintf(stderr,
		    ...
		    libstr,i,n1,aa1[i],pst.nsq,lcont,ocont,aa1ptr+i);
	    aa1[i]=0;
	    n1=i-1;
	    break;
	  }
