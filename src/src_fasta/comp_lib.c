...
/* $Name: fa34t25d9 $ - $Id: comp_lib.c,v 1.88 2006/05/31 18:43:39 wrp Exp $ */
...
main (int argc, char *argv[]) 
{
  ...
  int maxn = MAXTOT;		/* size of the library sequence examined */
  int maxl = MAXTOT;		/* size of library buffer */
  ...
  if (aa0[0]==NULL) {
    if ((aa0[0] = (unsigned char *)malloc((MAXTOT+1+32)*sizeof(unsigned char)))
	== NULL)
  ...
  */
  if ((aa1s = (unsigned char *)malloc((MAXTOT+1+32)*sizeof (char))) == NULL) {
...
#if !defined(__MWERKS__) || defined(_CARBON)         
...
#else
  m_msg.n0 = getseq (m_msg.tname, qascii, aa0[0], MAXTOT,
...
l2:    fputs(iprompt2,stdout);
  ...
  */
  ...
  if (buf_siz < MAXTOT) buf_siz = MAXTOT;
  ...
  maxl = MAXTOT - m_msg.n0 -2;	/* maxn = max library sequence space */
...
#ifdef COMP_THR
  /* before I call last_calc/showbest/showalign,
     I need init_work() to get an f_str */
